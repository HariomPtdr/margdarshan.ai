...
Three providers behind one async send():
  - mock   : log-only (default, dev)
  - twilio : Twilio WhatsApp (sandbox or approved sender)
  - meta   : Meta WhatsApp Cloud API (production, requires HSM templates)

Selected via env var WHATSAPP_PROVIDER.
"""
...
class WhatsAppClient:
    ...
    async def send(self, to: str, body: str) -> Optional[str]:
        """Send a free-form WhatsApp message. Returns provider message SID/ID or None."""
        if not to:
            return None
